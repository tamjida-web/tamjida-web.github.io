...
$document.ready(function () {
  ...
  if (plugins.search.length || plugins.searchResults) {
    ...
    var defaultTemplate = '<p class="h7 search_title"><a target="_top" href="#{href}" class="search_link">#{title}</a></p>' +
    ...
    for (i = 0; i < plugins.owl.length; i++) {
      ...
      for (j = 0; j < values.length; j++) {
        ...
        for (k = j; k >= -1; k--) {
          ...
          if (!responsive[values[j]]["dotsEach"] && c.attr("data" + aliaces[k] + "dots-each")) {
            responsive[values[j]]["dotsEach"] = k < 0 ? 1 : parseInt(c.attr("data" + aliaces[k] + "dots-each"));
          }
